...
for (var i = 0; i < products.length; i++) {
  ...
  var categoryText = category.innerText.toLowerCase().replace(/\s+/g, "");
...
document.getElementById("btnCheckout").addEventListener("click", function () {
  const itemEls = document.querySelectorAll("#cartItemsContainer .cart-item");
  if (itemEls.length === 0) {
  ...
  itemEls.forEach(item => {
    const nama = item.querySelector(".item-name")?.textContent || "Produk";
    const qty = parseInt(item.querySelector(".quantity span")?.textContent || "1");
    const hargaText = item.querySelector(".item-price")?.textContent?.replace(/[^\d]/g, "") || "0";
    const harga = parseInt(hargaText);
    const subtotal = qty * harga;

    pesan += `- ${nama} x${qty} = Rp${subtotal.toLocaleString()}\n`;
    total += subtotal;
  });

  pesan += `\n*Total: Rp${total.toLocaleString()}*`;

  const nomorWA = "6283129316843"; // Nomor WhatsApp Koperasi
  const urlWA = `https://wa.me/${nomorWA}?text=${encodeURIComponent(pesan)}`;

  alert("Transaksi Berhasil! Mengirim ke WhatsApp...");
